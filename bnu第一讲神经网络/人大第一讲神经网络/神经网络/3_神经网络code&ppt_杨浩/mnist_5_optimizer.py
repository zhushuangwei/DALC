...
#2、神经网络-对比-optimizer=Adam()
#2.1、建立输入层与隐层1，其激活函数为sigmoid
model2 = Sequential()  
...
result2 = model2.predict_classes(X_test,verbose=0)

print('predict on Testing Set:',result2)

#绘制对比图--训练集上的每次poch时accuracy的变化
...
plt.ylabel("Accuracy")
plt.title("Training")
plt.legend(('SGD','','Adam'),bbox_to_anchor=(0.95,0.4))
plt.show()
