...
#1、神经网络-Activation-sigmoid
#1.1.1、建立输入层与隐层1，其激活函数为sigmoid
model1 = Sequential()  
model1.add(Dense(output_dim=100,input_dim=28*28))  
model1.add(Activation('sigmoid')) 
 
#1.1.2、建立隐层2，其激活函数为sigmoid
model1.add(Dense(output_dim=100))   
model1.add(Activation('sigmoid'))  

#1.1.3、建立隐层3，其激活函数为sigmoid
model1.add(Dense(output_dim=100))   
model1.add(Activation('sigmoid'))  

#1.1.4、建立隐层4，其激活函数为sigmoid
model1.add(Dense(output_dim=100))   
model1.add(Activation('sigmoid')) 

#1.1.5、建立隐层5，其激活函数为sigmoid
model1.add(Dense(output_dim=100))   
model1.add(Activation('sigmoid')) 

#1.1.6、建立隐层6，其激活函数为sigmoid
model1.add(Dense(output_dim=100))   
model1.add(Activation('sigmoid')) 

#1.1.7、建立隐层7，其激活函数为sigmoid
model1.add(Dense(output_dim=100))   
model1.add(Activation('sigmoid')) 
...
#2、神经网络-对比-relu
#2.1.1、建立输入层与隐层1，其激活函数为relu
model2 = Sequential()  
model2.add(Dense(output_dim=100,input_dim=28*28))  
model2.add(Activation('relu')) 

#2.1.2、建立隐层2，其激活函数为relu
model2.add(Dense(output_dim=100))   
model2.add(Activation('relu'))  

#2.1.3、建立隐层3，其激活函数为relu
model2.add(Dense(output_dim=100))   
model2.add(Activation('relu'))  

#2.1.4、建立隐层4，其激活函数为relu
model2.add(Dense(output_dim=100))   
model2.add(Activation('relu')) 

#2.1.5、建立隐层5，其激活函数为relu
model2.add(Dense(output_dim=100))   
model2.add(Activation('relu')) 

#2.1.6、建立隐层6，其激活函数为relu
model2.add(Dense(output_dim=100))   
model2.add(Activation('relu')) 

#2.1.7、建立隐层7，其激活函数为relu
model2.add(Dense(output_dim=100))   
model2.add(Activation('relu')) 

#2.2、建立输出层，激活函数为softmax
model2.add(Dense(output_dim=10))  
model2.add(Activation('softmax'))

#2.3、使用compile来对学习过程进行配置
...
plt.ylim(0.0,1.0)
plt.title("Training")
plt.legend(('Sigmoid','','Relu'),bbox_to_anchor=(0.95,0.5))
plt.show()
